...
def _scene_compile() -> ImageClip:
    lines = [
        ...
        [],
        [("  ● ", T.CYAN), ("Scanning /raw for source files...", T.WHITE)],
        [("    ✓ ", T.GREEN), ("Attention.pdf", T.WHITE), ("                   (2.1 MB)", T.DIM)],
        [("    ✓ ", T.GREEN), ("FMs.pdf", T.WHITE), ("                         (14 MB)", T.DIM)],
        [("    ✓ ", T.GREEN), ("gpt3.pdf", T.WHITE), ("                        (6.5 MB)", T.DIM)],
        [("    ✓ ", T.GREEN), ("RNN and LSTM Basics.pptx", T.WHITE), ("       (5.0 MB)", T.DIM)],
        [("    ✓ ", T.GREEN), ("transformer-architecture.html", T.WHITE), ("   (698 KB)", T.DIM)],
        [("    ✓ ", T.GREEN), ("SLL.pdf", T.WHITE), ("                         (325 KB)", T.DIM)],
        [("    ✓ ", T.GREEN), ("bert-overview.txt", T.WHITE), ("               (49 KB)", T.DIM)],
        [("    ✓ ", T.GREEN), ("rlhf-overview.docx", T.WHITE), ("              (37 KB)", T.DIM)],
        [("    ✓ ", T.GREEN), ("llm-spreadsheet-tasks.xlsx", T.WHITE), ("      (21 KB)", T.DIM)],
        [],
        [("  ● ", T.CYAN), ("Extracting concepts → creating entity pages...", T.WHITE)],
        [("    ✓ ", T.GREEN), ("wiki/transformer.md", T.WHITE), ("         Transformer architecture (Vaswani 2017)", T.DIM)],
        [("    ✓ ", T.GREEN), ("wiki/self-attention.md", T.WHITE), ("      Scaled dot-product attention", T.DIM)],
        [("    ✓ ", T.GREEN), ("wiki/bert.md", T.WHITE), ("                BERT: deep bidirectional pre-training", T.DIM)],
        [("    ✓ ", T.GREEN), ("wiki/gpt-3.md", T.WHITE), ("               GPT-3: few-shot language model (175B)", T.DIM)],
        [("    ✓ ", T.GREEN), ("wiki/foundation-model.md", T.WHITE), ("    Foundation Models (Bommasani et al.)", T.DIM)],
        [("    ✓ ", T.GREEN), ("wiki/rlhf.md", T.WHITE), ("                RLHF: 3-stage SFT → RM → PPO pipeline", T.DIM)],
        [("    ✓ ", T.GREEN), ("wiki/scaling-laws.md", T.WHITE), ("        Neural scaling laws", T.DIM)],
        [("    ✓ ", T.GREEN), ("wiki/lstm.md", T.WHITE), ("                LSTM gated recurrent network", T.DIM)],
        [("    ✓ ", T.GREEN), ("wiki/small-language-model.md", T.WHITE), (" SLMs for agentic AI (≤10B params)", T.DIM)],
        [("    ", T.DIM), ("... (28 more entity pages)", T.DIM)],
        [],
        [("  ✓ ", T.GREEN), ("Compiled 9 sources → ", T.WHITE), ("37 entity pages", T.CYAN), ("  · index and log updated", T.DIM)],
    ]
...
def _scene_sync() -> ImageClip:
    lines = [
        ...
        [],
        [("  ● ", T.CYAN), ("Checking /raw for new or updated sources...", T.WHITE)],
        [("    ✓ ", T.DIM), ("Attention.pdf", T.DIM), ("                   already in log (2026-04-25)", T.DIM)],
        [("    ✓ ", T.DIM), ("FMs.pdf", T.DIM), ("                         already in log (2026-04-25)", T.DIM)],
        [("    ✓ ", T.DIM), ("gpt3.pdf", T.DIM), ("                        already in log (2026-04-25)", T.DIM)],
        [("    ✓ ", T.DIM), ("RNN and LSTM Basics.pptx", T.DIM), ("       already in log (2026-04-25)", T.DIM)],
        [("    ✓ ", T.DIM), ("transformer-architecture.html", T.DIM), ("   already in log (2026-04-25)", T.DIM)],
        [("    ✓ ", T.DIM), ("SLL.pdf", T.DIM), ("                         already in log (2026-04-25)", T.DIM)],
        [("    ✓ ", T.DIM), ("bert-overview.txt", T.DIM), ("               already in log (2026-04-25)", T.DIM)],
        [("    ✓ ", T.DIM), ("rlhf-overview.docx", T.DIM), ("              already in log (2026-04-25)", T.DIM)],
        [("    ✓ ", T.DIM), ("llm-spreadsheet-tasks.xlsx", T.DIM), ("      already in log (2026-04-25)", T.DIM)],
        [],
        [("  ✓ ", T.GREEN), ("All 9 sources already compiled. Wiki is up to date.", T.WHITE)],
        [("    ", T.DIM), ("37 entity pages current  ·  last updated 2026-04-25", T.DIM)],
        [],
        [("    ", T.DIM), ("To sync new sources: drop files into /raw and re-run /sync-wiki", T.DIM)],
    ]
...
def _scene_audit() -> ImageClip:
    lines = [
        ...
        [("    Checking inbound links, [[wikilinks]], cross-references, claim freshness...", T.DIM)],
        [],
        [("  AUDIT REPORT ", T.WHITE), ("─" * 56, T.BORDER)],
        [("    Pages audited:      ", T.DIM), ("37", T.WHITE)],
        [("    Links resolved:     ", T.DIM), ("252 / 252", T.GREEN), ("  (100%)", T.DIM)],
        ...
        [],
        [("  Link density — top backlinked pages:", T.DIM)],
        [("     transformer.md        ", T.WHITE), ("18 backlinks", T.CYAN)],
        [("     foundation-model.md   ", T.WHITE), ("15 backlinks", T.CYAN)],
        [("     gpt-3.md              ", T.WHITE), ("14 backlinks", T.CYAN)],
        [("     bert.md               ", T.WHITE), ("13 backlinks", T.CYAN)],
        [("     self-attention.md     ", T.WHITE), (" 9 backlinks", T.CYAN)],
        [],
        [("  ✓ ", T.GREEN), ("Wiki is healthy — no issues found", T.WHITE)],
        [("    ", T.DIM), ("37 pages  ·  252 links  ·  fully interconnected", T.DIM)],
    ]
